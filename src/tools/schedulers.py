...
def cosine_lr(args, batch_num):
    ...
    for epoch in range(args.epochs):
        for batch in range(batch_num):
            learning_rate.append(_lr_adjuster(epoch + batch / batch_num))
    if args.cooldown_length > 0:
        learning_rate += [args.min_lr]*args.cooldown_length*batch_num
    learning_rate = np.clip(learning_rate, args.min_lr, max(learning_rate))
    return learning_rate
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--cooldown_length', type=int, default=10, help='')
    args = argparser.parse_args()
    learning_rate = cosine_lr2(args, 2)
