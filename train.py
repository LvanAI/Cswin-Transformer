...
from src.tools.model_ema import ModelEMA
...
def main():
    ...
    net = get_model(args)
    n_parameters = sum(ops.Size()(p) for p in net.get_parameters() if p.requires_grad)
    ...
    loss_cb = VarMonitor(batch_num, 200, lr = learning_rate)
    ...
    eval_cb = EvaluateCallBack(model, eval_dataset=data.val_dataset, src_url=ckpt_save_dir,
                               ...
                               step_size = batch_num, n_parameters = n_parameters, lr = learning_rate,
                               rank_id = rank, save_freq=args.save_every)

    print("begin train")
    model.train(int(args.epochs - args.start_epoch + args.cooldown_length), data.train_dataset,
                callbacks=[ckpoint_cb, loss_cb,time_cb,  eval_cb],
